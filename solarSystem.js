...
class ObjectGroup {
    constructor( index, title, radius, extra ) {
        ...
        const planet = ObjectGroup.createObject( title, new THREE.SpehereGeometry(radius, 64, 32 ) );


        planet.position.x += 8 * index;
        objectGroup.add( plante );
...
for (let [index, { title, radius, extra }] of planetsMap.entries()) {
...
const pointLight = new THREE.PointLight(oxffffff, 1 );
...
function animate() {
    ...
    planetsMap.get(OBJECTS.JUPITER).rotation.y += EARTH_YEAR / 2;
    planetsMap.get(OBJECTS.SATURN).rotation.y += EARTH_YEAR / 4;
    planetsMap.get(OBJECTS.URANUS).rotation.y += EARTH_YEAR / 8;
    planetsMap.get(OBJECTS.NEPTUNE).rotation.y += EARTH_YEAR / 16;
    planetsMap.get(OBJECTS.PLUTO).rotation.y += EARTH_YEAR / 32;
